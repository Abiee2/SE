...
const PreviewPanel = () => {
  ...
  useEffect(() => {
    ...
    // Listen for preview updates from Settings
    const handlePreviewUpdate = (e) => {
      if (e.detail) setSettings(e.detail);
    };

    window.addEventListener('preview-updated', handlePreviewUpdate);

    return () => {
      window.removeEventListener('preview-updated', handlePreviewUpdate);
    };
  }, []);
  ...
  const textSizeClass = settings?.textSize
    ? `text-size-${settings.textSize.toLowerCase().replace(' ', '-')}`
    : '';
        ...
        {!isApplied ? (
          <div className={`layout-container before-state ${textSizeClass} ${readableFontClass} ${simpleModeClass}`}>
            ...
            <button
              ...
              onClick={() => {
                // Load existing profile to avoid overwriting personal details
                ...
                const appliedProfile = {
                  settings: { ...existingProfile.settings, ...settings },  // Merge settings
                  firstName: existingProfile.firstName || existingProfile.settings?.firstName || '',
                  lastName: existingProfile.lastName || existingProfile.settings?.lastName || '',
                  number: existingProfile.number || existingProfile.settings?.number || '',
                  address: existingProfile.address || existingProfile.settings?.address || '',
                  appliedAt: new Date().toISOString(),
                  appliedApps: existingProfile.appliedApps || []
                };

                // Commit full profile to localStorage
                localStorage.setItem("uaps-profile", JSON.stringify(appliedProfile));
          ...
          <div className="layout-container after-state">
            <div className="simplified-hero">
              <div className="hero-content">
                ...
                <p className="profile-tag">
                  Accessibility Profile: <strong>Enhanced</strong>
                </p>
              </div>
            </div>
            ...
            <div className="action-row-split">
              <button className="btn-action applied-status" disabled>
                ✓ Applied
              </button>
              <button
                className="btn-action dashboard-btn"
